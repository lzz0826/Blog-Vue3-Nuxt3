<template>
  <div class="flex w-full flex-col items-center bg-white">
    <div class="item-center mt-8 flex w-full max-w-4xl flex-col px-8 md:items-start lg:px-0">
      ...
      <el-button type="primary" @click="creatArticles" >新增文章</el-button>
    </div>
...
// onBeforeMount(() => {
//   const dd = useLazyFetch('/api/article', {
//   initialCache: false
//    })
// })
...
function creatArticles(){
  navigateTo('/manage/articles/create')
}
...
watch(error, (newError) => {
  ...
  if (!newError) {
    ...
    pushNotify('error', '取得文章失敗', newError?.data?.message ?? '未知錯誤')
  }
})
